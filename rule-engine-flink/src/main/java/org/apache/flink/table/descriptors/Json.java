...
public class Json extends FormatDescriptor {
    ...
    protected Map<String, String> toFormatProperties() {
        ...
        if (schema != null) {
            ...
            /*String[] schemas = schema.replaceFirst("ROW<", "")
                    .replace(">","").split(",");
            for (int i = 0; i < schemas.length; i++) {
                properties.putString("schema."+i+".name", schemas[i].trim().split(" ")[0]);
                properties.putString("schema."+i+".type", schemas[i].trim().split(" ")[1]);
            }*/
        }
